package by.epam.programmingWithClass.agregationandcomposition.bankaccount;
...
class Client {
	...
	public void blockAccaunt(double balance) {
		for (Account account : accounts) {
			if (account.getBalance() < 0) {
				account.block();
			}
	...
	public void unlockAccaunt(double balance) {
		for (Account account : accounts) {
			if (account.getBalance() > 0) {
				account.unlock();
			}
